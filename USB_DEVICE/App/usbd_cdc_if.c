...
void USB_SendBlock(uint8_t* data, uint32_t len) {
  for(uint32_t i = 0; i < len; i+=APP_TX_DATA_SIZE) {
    if (len - i < APP_TX_DATA_SIZE) {
      while (CDC_Transmit_FS(&data[i], len - i) == USBD_BUSY);
      break;
    }
    while (CDC_Transmit_FS(&data[i], APP_TX_DATA_SIZE) == USBD_BUSY);
  }
}
